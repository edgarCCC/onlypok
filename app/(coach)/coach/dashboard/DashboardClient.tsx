...
const TYPE_COLORS: Record<string, string> = {
  formation: VIOLET, video: CYAN, coaching: '#f59e0b',
}
...
export default function DashboardClient({
  ...
  const [recentPurchases]           = useState(initialPurchases)
  ...
  const monthlyRevenue = Array.from({ length: 6 }).map((_, i) => {
    ...
    const rev   = recentPurchases
      ...
      .reduce((a, p) => a + (p.formations?.price ?? 0), 0)
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#050709', color: CREAM, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 1000, margin: '0 auto', padding: '40px' }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <h1 style={{ fontSize: 52, fontWeight: 700, color: CREAM, letterSpacing: '-1.5px', lineHeight: 1, fontFamily: 'var(--font-syne,sans-serif)', margin: 0 }}>{greeting}</h1>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
          <Link href="/coach/formations"  style={{ textDecoration: 'none' }}><KpiCard label="Revenus totaux"   value={`${totalRevenue}€`}                         sub="depuis le début"                 color={VIOLET}  icon={TrendingUp} /></Link>
          <Link href="/coach/formations" style={{ textDecoration: 'none' }}><KpiCard label="Élèves ce mois"   value={studentsMonth}                              sub={`${totalStudents} au total`}     color={CYAN}    icon={Users} /></Link>
                ...
                {coachingSessions.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                    {coachingSessions.slice(0, 6).map((p, i) => {
                      const uname  = p.profiles?.username ?? 'Élève'
                        ...
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, background: 'rgba(245,158,11,0.04)', border: '1px solid rgba(245,158,11,0.08)' }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                              <span style={{ fontSize: 12, fontWeight: 600, color: CREAM }}>{uname}</span>
