    ...
    <div style={{ minHeight: '100vh', background: '#07090e', color: CREAM }}>
      ...
      <section style={{ padding: '140px clamp(20px,5vw,80px) 80px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <Link href="/coach/formations" style={{ padding: '13px 28px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.12)', color: SILVER, textDecoration: 'none', fontSize: 14, fontWeight: 600 }}>
            Mes formations
          </Link>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <style>{`
            @media (max-width: 420px) {
              .ctrn-grid { grid-template-columns: 1fr !important; }
            }
          `}</style>
          <div className="ctrn-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px,1fr))', gap: 16 }}>
      ...
      <section style={{ padding: '0 clamp(20px,5vw,80px) 120px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', background: 'linear-gradient(135deg, rgba(6,182,212,0.1), rgba(124,58,237,0.08))', border: '1px solid rgba(6,182,212,0.2)', borderRadius: 24, padding: 'clamp(40px,5vw,64px)', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 16, color: SILVER, margin: '0 auto 32px', maxWidth: 480, lineHeight: 1.6 }}>
            Combine le Trainer avec tes formations pour offrir une progression 10× plus rapide à tes élèves.
          </p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/coach/formations" style={{ padding: '13px 28px', borderRadius: 10, background: VIOLET, color: '#fff', textDecoration: 'none', fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8 }}>
              Mes formations <ArrowRight size={15} />
            </Link>
            <Link href="/coach/dashboard" style={{ padding: '13px 28px', borderRadius: 10, border: '1px solid rgba(6,182,212,0.4)', color: CYAN, textDecoration: 'none', fontSize: 14, fontWeight: 600 }}>
              Tableau de bord
            </Link>
          </div>
        </div>
